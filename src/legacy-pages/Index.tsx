...
import { RO_COUNTRIES } from "@/data/cities-md";
...
const Index = ({ locale, messages, initialKp, initialWind, initialMag, initialScales, initialForecast3 }: IndexProps) => {
  ...
  const countryRegionList = locale === "pl" || locale === "ro" || locale === "hu" || locale === "bg"
    ...
    : [];
  const roCountryRegionGroups = locale === "ro"
    ? RO_COUNTRIES.map((country) => ({
        ...country,
        regions: countryRegionList.filter((region) => region.countrySlug === country.slug),
  ...
  return (
    <div className="official-home min-h-screen">
      ...
      <section
        ...
        aria-label={
          locale === "pl"
            ? "Pogoda kosmiczna w województwach Polski"
            : locale === "ro"
        ...
        <div className="official-home-panel p-5 sm:p-6 lg:p-7">
          ...
          {locale === "ro" ? (
            <div className="space-y-8">
              {roCountryRegionGroups.map((country) => (
