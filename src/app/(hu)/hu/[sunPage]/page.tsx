...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const dayWord = parsed.mode === "tomorrow" ? "holnap" : "ma";
  const title =
    parsed.kind === "sunrise"
      ? `Napkelte Magyarországon ${dayWord} — időpontok városonként`
      : `Napnyugta Magyarországon ${dayWord} — időpontok városonként`;
  const description =
    parsed.kind === "sunrise"
      ? `Napkelte Magyarországon ${dayWord}: időpontok városok szerint, hajnal, napnyugta és nappal hossza.`
      : `Napnyugta Magyarországon ${dayWord}: időpontok városok szerint, alkonyat, éjszaka hossza és következő napkelte.`;
  const canonical = `/hu/${sunPage}`;
...
export default async function HungarianSunPage({ params }: Params) {
  const { sunPage } = await params;
  const parsed = parseSunPage(sunPage);
