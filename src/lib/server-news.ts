...
const NEWS_LOCALES: SiteLocale[] = ["uk", "ru", "pl", "ro", "hu", "bg", "cs", "en"];
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, published_at, image_url, status, title_uk, slug_uk, meta_description_uk, title_ru, slug_ru, meta_description_ru, title_pl, slug_pl, meta_description_pl, title_ro, slug_ro, meta_description_ro, title_hu, slug_hu, meta_description_hu"
        + ", title_bg, slug_bg, meta_description_bg, title_cs, slug_cs, meta_description_cs, title_en, slug_en, meta_description_en"
    )
    .eq("status", "published")
...
export async function getNewsArticleFallbackTargetBySlug(
  ...
  const { data, error } = await supabase
    .from("news")
    .select(
      "slug_uk, title_uk, content_uk, slug_ru, title_ru, content_ru, slug_pl, title_pl, content_pl, slug_ro, title_ro, content_ro, slug_hu, title_hu, content_hu, slug_bg, title_bg, content_bg, slug_cs, title_cs, content_cs, slug_en, title_en, content_en",
    )
    .or(NEWS_LOCALES.map((itemLocale) => `slug_${itemLocale}.eq.${slug}`).join(","))
    .eq("status", "published")
    .neq("source", "telegram_ai")
  ...
  const locales = [preferredLocale, "uk", "ru", "en", "pl", "ro", "hu", "bg", "cs"] satisfies SiteLocale[];
